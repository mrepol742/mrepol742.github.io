<noscript><iframe src="https://www.googletagmanager.com/ns.html?id=GTM-TFD9TXR" height="0" width="0" style="display:none;visibility:hidden"></iframe></noscript>
<div class="bg-primary p-2 shadow text-center text-white fs-6">
    Shop with ease, quality and affordable prices at <a href="https://ulishastore.com?utm_source=mrepol742&utm_medium=github&utm_campaign=referral&utm_content=navbanner">Ulisha Store</a>!
</div>
<header class="autohide navbar navbar-expand-lg fixed-top scrolled-up scrolled-up-tr">
    <nav class="container-fluid">
        <a class="navbar-brand" href="/">
            <img src="/favicon.png" alt="Melvin Jones Favicon" class="d-inline-block align-text-top" width="24" height="24">
            Melvin J.
        </a>
        <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarText" aria-controls="navbarText" aria-expanded="false" aria-label="Toggle navigation" id="qwe111">
            <span class="navbar-toggler-icon" id="qwe"></span>
        </button>
        <div class="collapse navbar-collapse" id="navbarText">
            <ul class="navbar-nav me-auto mb-2 mb-lg-0">
                <li class="nav-item">
                    <a class="nav-link" href="/">Home</a>
                </li>
                <li class="nav-item dropdown">
                    <a class="nav-link dropdown-toggle" href="#" id="navbarDropdownMenuLink" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                        Webvium
                    </a>
                    <div class="dropdown-menu" aria-labelledby="navbarDropdownMenuLink">
                        <a class="dropdown-item" href="/webvium/">Webvium Browser</a>
                        <a class="dropdown-item" href="/webviumbeta/">Webvium Beta</a>
                        <a class="dropdown-item" href="/webviumdev/">Webvium Dev</a>
                        <a class="dropdown-item" href="/search/">Webvium Search</a>
                        <a class="dropdown-item" href="/webviumvpn/">Webvium VPN</a>
                    </div>
                </li>
                <li class="nav-item">
                    <a class="nav-link" href="/project-orion/">Orion</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link" href="/link-tree/">Linktree</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link" href="https://mrepol742-blog.vercel.app">Post</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link" href="https://mrepol742-read-cv.vercel.app">Read CV</a>
                </li>
            </ul>
            <div>
                <ul class="nav navbar-nav" style="flex-direction: row;">
                    <li class="nav-item">
                        <a class="nav-link" href="https://github.com/mrepol742" title="Github">
                            <i class="fa-brands fa-github"></i>
                        </a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link" href="https://mrepol742.github.io/sitemap" title="Sitemap">
                            <i class="fa-solid fa-sitemap"></i>
                        </a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link" href="https://mrepol742.github.io/rss" title="RSS">
                            <i class="fa-solid fa-rss"></i>
                        </a>
                    </li>
                </ul>
            </div>

        </div>
    </nav>
</header>
<?php include("components/noscript.php") ?>
